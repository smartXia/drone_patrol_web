...
export async function scan({ cursor = '0', match, count = 100, type }) {
  const { data } = await client.post('/scan', withConn({ key: match, cursor, count, type }))
  return data
}
...
export async function keyMetadata(key) {
  const { data } = await client.post('/metadata', withConn({ key }))
  return data.data
}
